			...
			new Vue({
				...
				methods: {
					fetchAll(){
						...
						const base = new Airtable({apiKey: 'keyTJxXP7qFvC07mg'}).base('appMbaR8BICkWblB3');
						...
						base('news').select({
							...
							records.forEach((record) => {
								items_news.push(record.fields)
							});
							fetchNextPage();
						}, done = (err) => {
							if (err) { console.error(err); return; }
							this.news = items_news[0]
						});

						base('read').select({
						}).eachPage(page = (records, fetchNextPage) => {
							records.forEach((record) => {
								items_read.push(record.fields)
							});
							fetchNextPage();
						}, done = (err) => {
							if (err) { console.error(err); return; }
							this.read = items_read
						});

						base('data').select({
						}).eachPage(page = (records, fetchNextPage) => {
							records.forEach((record) => {
								items_data.push(_.get(record, 'fields'))
							});
							fetchNextPage();
						}, done = (err) => {
							if (err) { console.error(err); return; }
							this.dataCSV = items_data

							$('.loading span').text('載入中 [ 1/3 ]')
							this.dictArrCSV = this.dataCSV.reduce((dict, item, index) => {
								let key = item.id
								delete item.id
								dict[key] = item
								return dict
						...
						try {
							...
							TimeLine = new TimelineMax()
								.add(()=>{
									...
									this.handleMap = this.dataMap.map(item => {
						...
						this.geojson.eachLayer(function (layer) {
							...
							switch(date){
								...
								case '18':
									...
									break;
								case '19':
									$confirmed = layer.feature.properties.confirmed_latest;
									$death = layer.feature.properties.death_latest;
									break;
							}
							if($confirmed > 0){
								let $note
								if(layer.feature.properties.note === undefined){
									$note = ''
								} else {
									$note = layer.feature.properties.note
								}
								layer.bindTooltip('<h6>' + layer.feature.properties.name_zh + '</h6><div class="confirmed">確診 ' + numberWithCommas($confirmed) + ' 例</div><div class="death">死亡 ' + numberWithCommas($death) + ' 例</div><div class="note">' + $note + '</div>', {
									direction: 'right',
									permanent: false,
									sticky: true,
									offset: [10, 0],
					...
					setMarker(date){
						...
						switch(date){
							...
							case '18':
								...
								break;
							case '19':
								$confirmed = this.news.diamond_princess_confirmed;
								$death = this.news.diamond_princess_death
								break;
						}
						if(Number(date) >= 4){
					...
					style(feature) {
						...
						switch(this.date){
							...
							case '18':
								...
								break;
							case '19':
								$color = feature.properties.confirmed_latest;
								break;
						}
						return {
							weight: 1,
				...
				computed: {
					groupByCategory(){
						return _.chain(this.read)
								.groupBy('category_name')
								.value()
					},
					groupByType(){
						return _.chain(this.dataCSV)
								.filter(item => item.confirmed_latest !== 0)
								.orderBy('confirmed_latest', 'desc')
								.groupBy('type')
								.value()
					},
					isMobile(){
				...
				filters: {
					sumData(array, target){
						...
						for(let i in array){
							if(target === 'confirmed'){
								sum += array[i].confirmed_latest;
							} else if(target === 'death'){
								sum += array[i].death_latest;
							} else if(target === 'cure'){
								sum += array[i].cure_latest;
							}
						}
						return sum
					},
