...
var Global = function () {
	var addMember = function(){
		...
		scene = new ScrollMagic.Scene({ triggerElement: "#footer", triggerHook: 'onEnter', offset: 100})
			.setTween(
				TweenLite.to('#footer .member', .5, {y: 0, ease: Power3.easeOut})
			)
			// .addIndicators()
			.addTo(controller)
	}
			...
			new Vue({
				...
				data: {
					...
					filterData: [
						{
							'date': '12.31',
							'text': '武漢衛健委首發疫情公告，稱未發現「明顯人傳人」；港府加強出入境口岸健康監察' 
						},{
							'date': '1.20',
							'text': '中國工程院院士鍾南山表示2019冠狀病毒「人傳人」；台灣成立「中央流行疫情指揮中心」' 
						},{
							'date': '1.21',
							'text': '台灣出現首例確診。翌日，港澳分別出現首例確診' 
						},{
							'date': '1.23',
							'text': '中國武漢市宣佈封城' 
						},{
							'date': '2.5',
							'text': '香港2.3出現首例死亡；鑽石公主號確診10例' 
						},{
							'date': '2.17',
							'text': '台灣2.15出現首例死亡；鑽石公主號確診累計454例' 
						},{
							'date': '2.19',
							'text': '韓國在大邱市某教堂發現超級傳播事件，兩日新增百例確診' 
						},{
							'date': '3.4',
							'text': '韓國確診超過5600例；全球超過80個國家及地區出現確診個案'
						},{
							'date': '3.7',
							'text': '全球確診個案超過10萬'
						},{
							...
							'date': '3.26',
							'text': '美國確診個案超過8.2萬，超過中國與意大利'
						},{
							'date': '4.3',
							'text': '全球確診個案超過100萬'
						},{
							'date': '5.21',
							'text': '全球確診個案超過500萬；美國累計確診破155萬'
						},{
							'date': '6.28',
							'text': '全球確診個案超過1000萬'
						},{
							'date': '8.10',
							'text': '全球確診個案超過2000萬；美國、印度和巴西的個案占全球確診總數一半以上'
						},{
							'date': '9.18',
							'text': '全球確診個案超過3000萬'
						},{
							...
							'date': '10.19',
							'text': '全球確診個案超過4000萬；美國、印度和巴西的確診數為全球前三'
						},{
							'date': '11.9',
							'text': '全球確診個案超過5000萬'
						},{
				...
				methods: {
					async fetchNews(){
						try {
							const res = await $.get('https://spreadsheets.google.com/feeds/list/1az3e1HrKoe1kf2oa5AcKcTOd5UP3Sy-zRzc8RvfKJjA/1/public/values?alt=json')
							const data = res.feed.entry
							this.newsTitle = data[0].gsx$title.$t
                            this.newsLink = data[0].gsx$link.$t
                            this.newConfirmed = data[0].gsx$newconfirmed.$t
                            this.newDeath = data[0].gsx$newdeath.$t
                            this.newCure = data[0].gsx$newcure.$t
							
							this.updateTime = data[0].gsx$updatetime.$t
							this.dpConfirmed = parseInt(data[0].gsx$diamondprincessconfirmed.$t)
							this.dpDeath = parseInt(data[0].gsx$diamondprincessdeath.$t)
							this.dpCure = parseInt(data[0].gsx$diamondprincesscure.$t)
						} catch(e) {
							console.error(e)
						} finally {
						}
					...
					async fetchRead(){
						try {
							const res = await $.get('https://spreadsheets.google.com/feeds/list/1az3e1HrKoe1kf2oa5AcKcTOd5UP3Sy-zRzc8RvfKJjA/2/public/values?alt=json')
							const data = res.feed.entry
							const items = []
							for(var i in data) {
								let item = {};
								item.category_id = data[i].gsx$categoryid.$t;
								item.category_name = data[i].gsx$categoryname.$t;
								item.title = data[i].gsx$title.$t;
								item.link = data[i].gsx$link.$t;
								item.thumbnail = data[i].gsx$thumbnail.$t;
								items.push(item);
							}
							this.readMore = items
						} catch(e) {
					...
					async fetchCSV(){
						try {
							const res = await $.get('https://spreadsheets.google.com/feeds/list/1az3e1HrKoe1kf2oa5AcKcTOd5UP3Sy-zRzc8RvfKJjA/3/public/values?alt=json')
							const data = res.feed.entry
							const items = [];
							for(var i in data) {
								let item = {};
								item.id = parseInt(data[i].gsx$id.$t);
								item.type = data[i].gsx$type.$t;
								item.name_zh = data[i].gsx$namezh.$t;
								item.note = data[i].gsx$note.$t;
								item.confirmed = parseInt(data[i].gsx$confirmedlatest.$t);
								item.death = parseInt(data[i].gsx$deathlatest.$t);
								item.cure = parseInt(data[i].gsx$curelatest.$t);
								item.confirmed_201109 = parseInt(data[i].gsx$confirmed201109.$t);
								item.death_201109 = parseInt(data[i].gsx$death201109.$t);
                                item.confirmed_201019 = parseInt(data[i].gsx$confirmed201019.$t);
								item.death_201019 = parseInt(data[i].gsx$death201019.$t);
                                item.confirmed_200928 = parseInt(data[i].gsx$confirmed200928.$t);
								item.death_200928 = parseInt(data[i].gsx$death200928.$t);
                                item.confirmed_200918 = parseInt(data[i].gsx$confirmed200918.$t);
								item.death_200918 = parseInt(data[i].gsx$death200918.$t);
                                item.confirmed_200810 = parseInt(data[i].gsx$confirmed200810.$t);
								item.death_200810 = parseInt(data[i].gsx$death200810.$t);
								item.confirmed_200628 = parseInt(data[i].gsx$confirmed200628.$t);
								item.death_200628 = parseInt(data[i].gsx$death200628.$t);
								item.confirmed_200521 = parseInt(data[i].gsx$confirmed200521.$t);
								item.death_200521 = parseInt(data[i].gsx$death200521.$t);
								item.confirmed_200403 = parseInt(data[i].gsx$confirmed200403.$t);
								item.death_200403 = parseInt(data[i].gsx$death200403.$t);
								item.confirmed_200326 = parseInt(data[i].gsx$confirmed200326.$t);
								item.death_200326 = parseInt(data[i].gsx$death200326.$t);
								item.confirmed_200318 = parseInt(data[i].gsx$confirmed200318.$t);
								item.death_200318 = parseInt(data[i].gsx$death200318.$t);
								item.confirmed_200307 = parseInt(data[i].gsx$confirmed200307.$t);
								item.death_200307 = parseInt(data[i].gsx$death200307.$t);
								item.confirmed_200304 = parseInt(data[i].gsx$confirmed200304.$t);
								item.death_200304 = parseInt(data[i].gsx$death200304.$t);
								item.confirmed_200219 = parseInt(data[i].gsx$confirmed200219.$t);
								item.death_200219 = parseInt(data[i].gsx$death200219.$t);
								item.confirmed_200217 = parseInt(data[i].gsx$confirmed200217.$t);
								item.death_200217 = parseInt(data[i].gsx$death200217.$t);
								item.confirmed_200205 = parseInt(data[i].gsx$confirmed200205.$t);
								item.death_200205 = parseInt(data[i].gsx$death200205.$t);
								item.confirmed_200123 = parseInt(data[i].gsx$confirmed200123.$t);
								item.death_200123 = parseInt(data[i].gsx$death200123.$t);
								item.confirmed_200121 = parseInt(data[i].gsx$confirmed200121.$t);
								item.death_200121 = parseInt(data[i].gsx$death200121.$t);
								item.confirmed_200120 = parseInt(data[i].gsx$confirmed200120.$t);
								item.death_200120 = parseInt(data[i].gsx$death200120.$t);
								item.confirmed_191231 = parseInt(data[i].gsx$confirmed191231.$t);
								item.death_191231 = parseInt(data[i].gsx$death191231.$t);
								items.push(item)
                            }
							this.dataCSV = items
						} catch(e) {
							...
							$('.loading span').text('載入中 [ 1/3 ]')
							const arrCSV = this.dataCSV;
							this.dictArrCSV = arrCSV.reduce((dict, item, index) => {
								let key = item.id
								delete item.id
								dict[key] = item
								return dict
					...
					async fetchMap(){
						try {
							const res = await $.get('data/merged.simplify.json')
							this.dataMap = res.features
						} catch(e) {
							console.error(e)
						} finally {
							TimeLine = new TimelineMax()
								.add(()=>{
									$('.loading span').text('載入中 [ 2/3 ]')
									const dataMap = this.dataMap 
									this.handleMap = dataMap.map(item => {
										var props = item.properties;
										var neid = props.NE_ID;
										if (this.dictArrCSV.hasOwnProperty(neid)) {
											var handleMapItem = this.dictArrCSV[neid];
											Object.assign(props, handleMapItem);
										}
										return item;
									})
								}, 1.5)
								.add(()=>{
									$('.loading span').text('載入中 [ 3/3 ]')
									this.setMap()
									this.setCountry(this.date)
									this.setMarker(this.date)
									this.loadingDone()
                                }, 2)
                                .add(()=>{
                                    window.scrollTo(0, 0)
					...
					initIndex(){
						...
						introTL = new TimelineMax()
							.to('.intro', .5, {alpha: 0, ease: Power3.easeOut}, 0)
							.set('.intro', {className: '+=active'}, .5)
							.set('.intro', {display: 'none'})
						scene = new ScrollMagic.Scene({ triggerElement: ".scene", triggerHook: 'onLeave', offset: 1})
							.setTween(introTL)
							.addTo(controller)
						scene = new ScrollMagic.Scene({ triggerElement: ".map", triggerHook: 'onLeave', duration: this.window.height * 0.5})
							.setPin('.map')
							.addTo(controller)
						scene = new ScrollMagic.Scene({ triggerElement: ".statistic", triggerHook: 'onEnter', offset: 100})
							.setClassToggle(".map .anchor", "active")
							.setTween(
								TweenLite.to('.map .anchor', .5, {alpha: 0})
							)
							.addTo(controller)
						TweenLite.from('.loading .container', 1, {alpha: 0, ease: Power3.easeOut})
					},
					setMap(){
						const LeafIcon = L.Icon.extend({ options: {iconSize: [35, 35]} });
						const ferryIcon = new LeafIcon({iconUrl: 'img/ferry.svg'});

						this.map = L.map('leaflet').setView([41.9102415, 12.3959153], 3)
						...
						this.dpMarker = L.marker([34.548471, 139.653540], {icon: ferryIcon}).addTo(this.map)
					},
					setCountry(date){
						this.map.removeLayer(this.geojson);

						this.geojson = L.geoJson(this.handleMap, {
							style: this.style,
						...
						this.geojson.eachLayer(function (layer) {
							...
							switch(date){
								case '0':
									$confirmed = layer.feature.properties.confirmed_191231;
									$death = layer.feature.properties.death_191231;
									break;
								case '1':
									$confirmed = layer.feature.properties.confirmed_200120;
									$death = layer.feature.properties.death_200120;
									break;
								case '2':
									$confirmed = layer.feature.properties.confirmed_200121;
									$death = layer.feature.properties.death_200121;
									break;
								case '3':
									$confirmed = layer.feature.properties.confirmed_200123;
									$death = layer.feature.properties.death_200123;
									break;
								case '4':
									$confirmed = layer.feature.properties.confirmed_200205;
									$death = layer.feature.properties.death_200205;
									break;
								case '5':
									$confirmed = layer.feature.properties.confirmed_200217;
									$death = layer.feature.properties.death_200217;
									break;
								case '6':
									$confirmed = layer.feature.properties.confirmed_200219;
									$death = layer.feature.properties.death_200219;
									break;
								case '7':
									$confirmed = layer.feature.properties.confirmed_200304;
									$death = layer.feature.properties.death_200304;
									break;
								case '8':
									$confirmed = layer.feature.properties.confirmed_200307;
									$death = layer.feature.properties.death_200307;
									break;
								case '9':
									$confirmed = layer.feature.properties.confirmed_200318;
									$death = layer.feature.properties.death_200318;
									break;
								case '10':
									$confirmed = layer.feature.properties.confirmed_200326;
									$death = layer.feature.properties.death_200326;
									break;
								case '11':
									$confirmed = layer.feature.properties.confirmed_200403;
									$death = layer.feature.properties.death_200403;
									break;
								case '12':
									$confirmed = layer.feature.properties.confirmed_200521;
									$death = layer.feature.properties.death_200521;
									break;
								case '13':
									$confirmed = layer.feature.properties.confirmed_200628;
									$death = layer.feature.properties.death_200628;
									break;
								case '14':
									$confirmed = layer.feature.properties.confirmed_200810;
									$death = layer.feature.properties.death_200810;
									break;
								case '15':
									$confirmed = layer.feature.properties.confirmed_200918;
									$death = layer.feature.properties.death_200918;
									break;
								case '16':
									$confirmed = layer.feature.properties.confirmed_200928;
									$death = layer.feature.properties.death_200928;
									break;
								case '17':
									$confirmed = layer.feature.properties.confirmed_201019;
									$death = layer.feature.properties.death_201019;
									break;
								case '18':
									$confirmed = layer.feature.properties.confirmed_201109;
									$death = layer.feature.properties.death_201109;
									break;
								case '19':
									$confirmed = layer.feature.properties.confirmed;
									$death = layer.feature.properties.death;
									break;
							}
							if($confirmed > 0){
								layer.bindTooltip('<h6>' + layer.feature.properties.name_zh + '</h6><div class="confirmed">確診 ' + numberWithCommas($confirmed) + ' 例</div><div class="death">死亡 ' + numberWithCommas($death) + ' 例</div><div class="note">' + layer.feature.properties.note + '</div>', {
									direction: 'right',
									permanent: false,
									sticky: true,
									offset: [10, 0],
					...
					setMarker(date){
						var $confirmed, $death;
						this.map.removeLayer(this.dpMarker);
						switch(date){
							case '4':
								$confirmed = 10;
								$death = 0;
							...
							case '18':
								...
								break;
							case '19':
								$confirmed = this.dpConfirmed;
								$death = this.dpDeath
								break;
						}
						if(Number(date) >= 4){
							this.dpMarker.bindPopup('<h6>鑽石公主號</h6><div class="confirmed">確診 '+$confirmed+' 例</div><div class="death">死亡 '+$death+' 例</div><div class="note"></div>').addTo(this.map);
						}
					},
					style(feature) {
						var $color;
						switch(this.date){
							case '0':
								$color = feature.properties.confirmed_191231;
								break;
							case '1':
								$color = feature.properties.confirmed_200120;
								break;
							case '2':
								$color = feature.properties.confirmed_200121;
								break;
							case '3':
								$color = feature.properties.confirmed_200123;
								break;
							case '4':
								$color = feature.properties.confirmed_200205;
								break;
							case '5':
								$color = feature.properties.confirmed_200217;
								break;
							case '6':
								$color = feature.properties.confirmed_200219;
								break;
							case '7':
								$color = feature.properties.confirmed_200304;
								break;
							case '8':
								$color = feature.properties.confirmed_200307;
								break;
							case '9':
								$color = feature.properties.confirmed_200318;
								break;
							case '10':
								$color = feature.properties.confirmed_200326;
								break;
							case '11':
								$color = feature.properties.confirmed_200403;
								break;
							case '12':
								$color = feature.properties.confirmed_200521;
								break;
							case '13':
								$color = feature.properties.confirmed_200628;
								break;
							case '14':
								$color = feature.properties.confirmed_200810;
								break;
							case '15':
								$color = feature.properties.confirmed_200918;
								break;
							case '16':
								$color = feature.properties.confirmed_200928;
								break;
							case '17':
								$color = feature.properties.confirmed_201019;
								break;
							case '18':
								$color = feature.properties.confirmed_201109;
								break;
							case '19':
								$color = feature.properties.confirmed;
								break;
						}
						return {
							weight: 1,
					...
					mapButton(){
						TweenLite.to(window, 1, {scrollTo:".statistic", autoKill: false, ease: Power3.easeInOut});
					},
					loadingDone(){
						TimeLine = new TimelineMax()
							.to('.loading .container', 1, {alpha: 0, ease: Power3.easeIn})
							.to('.loading', 1, {alpha: 0, ease: Power3.easeIn}, 1)
							.set('.loading', {'display': 'none'}, 2)
					},
					filterDateDesktop(event){
						var $target = event.target.value.toString()
						TimeLine = new TimelineMax()
							.add(()=>{
								this.date = $target
								this.setCountry($target)
								this.setMarker($target)
								this.closePopup()
								$('.map .filter.desktop .tips').fadeOut(300)
							})
							.add(()=>{
								$('.map .filter.desktop .bar li').eq($target).find('p').fadeIn(300)
							})
					},
					filterDateMobile(event){
						var $target = event.target.value.toString()
						TimeLine = new TimelineMax()
							.add(()=>{
								this.date = $target
								this.setCountry($target)
								this.setMarker($target)
								if(event.target.value < 7 && this.flyPos === 'world'){
									this.map.flyTo([31.197769, 121.510137], 4, {
										animate: false,
										duration: 0
									});
									this.flyPos = 'china'
								} else if(event.target.value >= 7 && this.flyPos === 'china'){
									this.map.flyTo([41.9102415, 12.3959153], 3, {
										animate: false,
										duration: 0
									});
									this.flyPos = 'world'
					...
					closePopup(){
						$('.map .filter.desktop .bar p').fadeOut(300)
						this.filterDisplay(false)
					},
					filterDisplay(boolean){
						...
						if(boolean){
							TweenLite.to($('.map .filter'), .3, {x: -distance, ease: Power3.easeOut})
							$('.map .button.menu span').fadeOut(300)
						} else {
							TweenLite.to($('.map .filter'), .3, {x: distance, ease: Power3.easeIn})
						}
                    },
                    switchStatistic(){
				...
				computed: {
					groupByCategory(){
						return _.chain(this.readMore)
								.groupBy('category_name')
								.value()
					},
					groupByType(){
						return _.chain(this.dataCSV)
				...
				filters: {
					sumData(array, target){
						let sum = 0
						for(let i in array){
							if(target === 'confirmed'){
								sum += array[i].confirmed;
					...
					// formateTime(val){
					// 	return moment(String(val)).format('YYYY-MM-DD HH:mm:ss')
                    // },
					groupName(val){
						return val === 'America' ? '美洲' :
							   val === 'Asia' ? '亞洲（中國大陸境外）' :
							   val === 'Europe' ? '歐洲' :
							   val === 'Africa' ? '非洲' :
							   val === 'china' ? '亞洲（中國大陸境內）' :
